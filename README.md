# Elevates Labs Cybersecurity Internship
<img width="1920" height="1080" alt="banner" src="https://github.com/user-attachments/assets/9760f6c8-b190-49ad-bfa9-688393b03ddf"/>

Welcome to the Cybersecurity Internship Repository for the ElevateLabs Cybersec Intern Program.
This repository contains all tasks, documentation, and project outputs completed during the internship.

---

## 🚀 **Overview**

This repository is part of a 45-day cybersecurity internship focusing on hands-on learning across different security domains.
Each task folder contains:

- Detailed execution steps
- Screenshots
- Reports & analysis
- Insights learned during the challenge

---

## 📌 **Table of Content**

### 👉 **[Task 1](./Task%201/README.md)** 

### 👉 **[Task 2](./Task%202/README.md)**  

### 👉 **[Task 3](./Task%203/README.md)** 

### 👉 **[Task 4](./Task%204/README.md)** 

### 👉 **[Task 5](./Task%205/README.md)** 

### 👉 **[Task 6](./Task%206/README.md)**  

### 👉 **[Task 7](./Task%207/README.md)** 

---

## 🎯 **Objectives of the Internship**

- Develop practical cybersecurity investigation skills 
- Understand offensive & defensive security workflows 
- Apply OSINT and threat analysis techniques   
- Document findings professionally  
- Build a portfolio-ready project repository

---

## 🧾 **License**

This repository is created for educational and internship purposes under ElevateLabs.

---
